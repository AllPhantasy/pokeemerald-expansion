...
#define I_BERRY_EV_JUMP                 GEN_7  // In Gen4 only, EV-lowering Berries lower a stat's EV to 100 if it is above 100.
#define I_GRISEOUS_ORB_FORM_CHANGE      GEN_7  // In Gen9+, the Griseous Orb no longer changes Giratina's form when held.
#define I_GEM_BOOST_POWER               GEN_7  // In Gen6+, the Gem boost power was reduced from 50% to 30%.
...
#define I_TYPE_BOOST_POWER              GEN_7  // In Gen4+, all regular type boosting held items had their power increased from 10% to 20%. eg. Charcoal
...
#define I_ROTOM_CATALOG_THUNDER_SHOCK   GEN_7  // In Gen9+, reverting Rotom to its base form will teach it Thunder Shock even if it knows another move.
#define I_REPEL_INCLUDE_FAINTED         GEN_7  // In Gen1 and Gen6+, Repels always use the level of the first member of the party to check which wild Pokémon to prevent encounters with, even if that member is fainted. In Gen2-5, it only uses the level of the first non-fainted Pokémon.
...
#define I_FISHING_STICKY_BOOST     GEN_7      // In Gen 3, a Pokemon with Suction Cups or Sticky Hold in the first slot of the party causes the chance for a bite to increase by about 35%. In Gen 4 onwards, it doubles the base bite chance.
