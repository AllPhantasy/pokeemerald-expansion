...
#define B_MULTI_HIT_CHANCE          GEN_7 // In Gen5+, multi-hit moves have different %. See SetRandomMultiHitCounter for values.
...
#define B_LIGHT_BALL_ATTACK_BOOST   GEN_7 // In Gen4+, Light Ball doubles the power of physical moves in addition to special moves.
#define B_SANDSTORM_SPDEF_BOOST     GEN_7 // In Gen4+, Sandstorm weather multiplies the Sp. Defense of Rock-type Pokémon by x1.5.
...
#define B_UNEVOLVED_EXP_MULTIPLIER  GEN_7 // In Gen6+, if the Pokémon is at or past the level where it would be able to evolve, but it has not, it gets a ~1.2 multiplier to EXP gain. Only applies to Pokémon with EVO_LEVEL method.
...
// Damage settings
#define B_BURN_DAMAGE               GEN_7 // In Gen7+, burn damage is 1/16th of max HP instead of 1/8th. Also applies to Frostbite.
...
#define B_KNOCK_OFF_DMG             GEN_7 // In Gen6+, Knock Off deals 50% more damage when knocking off an item.
...
#define B_PARENTAL_BOND_DMG         GEN_7 // In Gen7+, Parental Bond's second hit does 25% of the initial hits damage. Before, it did 50%.
#define B_MULTIPLE_TARGETS_DMG      GEN_7 // In Gen4+, damage dealt by moves that hit multiple targets at once is reduced to 75%. Before, it was 50%.
...
#define B_UPDATED_TYPE_MATCHUPS     GEN_7 // Updates Type matchups. src/data/types_info.h for details.
...
#define B_ROOST_PURE_FLYING         GEN_7 // In Gen5+, Roost makes pure Flying-types into Normal-type.
#define B_STATUS_TYPE_IMMUNITY      GEN_7 // In Gen1, Pokémon were immune to paralysis/freeze/burn side effects of attacking moves, if they shared a type with the move.
...
#define B_UPROAR_IGNORE_SOUNDPROOF  GEN_7 // In Gen5+, Uproar status ignores Soundproof.
...
#define B_RECALC_TURN_AFTER_ACTIONS GEN_7 // In Gen8, switching/using a move affects the current turn's order of actions, better known as dynamic speed.
...
// Move data settings
#define B_UPDATED_MOVE_DATA         GEN_7 // Updates move data in gMovesInfo, including Power, Accuracy, PP, stat changes, targets and chances of secondary effects.
#define B_UPDATED_MOVE_TYPES        GEN_7 // Updates move types.
#define B_UPDATED_MOVE_FLAGS        GEN_7 // Updates move flags.
...
#define B_UPDATED_CONVERSION_2      GEN_7 // In Gen5+, Conversion 2 changes the user's type to a type that resists the last move used by the selected target. Before, it would consider the last move being successfully hit by. Additionally, Struggle is considered Normal type before Gen 5.
...
#define B_UPDATED_ABILITY_DATA      GEN_7 // Affects flags
...
#define B_KINGS_SHIELD_LOWER_ATK    GEN_7 // In Gen8+, it lowers Atk by 1 stage instead of 2 of opponents that hit it.
#define B_SPEED_BUFFING_RAPID_SPIN  GEN_7 // In Gen8, Rapid Spin raises the user's Speed by 1 stage.
...
#define B_FOCUS_ENERGY_CRIT_RATIO   GEN_7 // In Gen3+, Focus Energy increases critical hit ratio by 2 instead of 1.
...
#define B_METRONOME_MOVES           GEN_7 // This config will determine up to which generation will Metronome pull moves from.
#define B_TELEPORT_BEHAVIOR         GEN_7 // In LGPE onwards (Gen8+ here), Teleport allows the user to swap out with another party member.
#define B_BEAT_UP                   GEN_7 // In Gen5+, Beat Up uses a different formula to calculate its damage, and deals Dark-type damage. Prior to Gen 5, each hit also announces the party member's name.
#define B_DARK_VOID_FAIL            GEN_7 // In Gen7+, only Darkrai can use Dark Void.
#define B_BURN_HIT_THAW             GEN_7 // In Gen6+, damaging moves with a chance of burn will thaw the target, regardless if they're fire-type moves or not.
#define B_HEALING_WISH_SWITCH       GEN_7 // In Gen5+, the mon receiving Healing Wish is sent out at the end of the turn.
...
#define B_STOCKPILE_RAISES_DEFS     GEN_7 // In Gen4+, Stockpile also raises Defense and Sp. Defense stats. Once Spit Up / Swallow is used, these stat changes are lost.
#define B_TRANSFORM_SHINY           GEN_7 // In Gen4+, Transform will copy the shiny state of the opponent instead of maintaining its own shiny state.
#define B_TRANSFORM_FORM_CHANGES    GEN_7 // In Gen5+, Transformed Pokemon cannot change forms.
#define B_WIDE_GUARD                GEN_7 // In Gen5 only, Wide Guard has a chance to fail if used consecutively.
#define B_QUICK_GUARD               GEN_7 // In Gen5 only, Quick Guard has a chance to fail if used consecutively.
#define B_IMPRISON                  GEN_7 // In Gen5+, Imprison doesn't fail if opposing pokemon don't have any moves the user knows.
#define B_ALLY_SWITCH_FAIL_CHANCE   GEN_7 // In Gen9, using Ally Switch consecutively decreases the chance of success for each consecutive use.
#define B_SKETCH_BANS               GEN_7 // In Gen9+, Sketch is unable to copy more moves than in previous generations.
#define B_KNOCK_OFF_REMOVAL         GEN_7 // In Gen5+, Knock Off removes the foe's item instead of rendering it unusable.
#define B_HEAL_BELL_SOUNDPROOF      GEN_7 // In Gen5, Heal Bell affects all mons with Soundproof.  In Gen6-8 it affects inactive mons, but not battlers. In Gen9 it always affects the user.
#define B_CHARGE                    GEN_7 // In Gen8-, Charge status is lost regardless of the typing of the next move.
#define B_POWDER_RAIN               GEN_7 // In Gen7+, Powder doesn't damage the user of a Fire type move in heavy rain.
#define B_AFTER_YOU_TURN_ORDER      GEN_7 // In Gen8+, After You doesn't fail if the turn order wouldn't change after use.
#define B_QUASH_TURN_ORDER          GEN_7 // In Gen8+, Quash-affected battlers move according to speed order. Before Gen8, Quash-affected battlers move in the order they were affected by Quash.
#define B_DESTINY_BOND_FAIL         GEN_7 // In Gen7+, Destiny Bond fails if used repeatedly.
#define B_PURSUIT_TARGET            GEN_7 // In Gen4+, Pursuit attacks a switching opponent even if they weren't targeting them. Before Gen4, Pursuit only attacks a switching opponent that it originally targeted.
...
#define B_SYNCHRONIZE_TOXIC         GEN_7 // In Gen5+, if a Pokémon with Synchronize is badly poisoned, the opponent will also become badly poisoned. Previously, the opponent would become regular poisoned.
#define B_UPDATED_INTIMIDATE        GEN_7 // In Gen8, Intimidate doesn't work on opponents with the Inner Focus, Scrappy, Own Tempo or Oblivious abilities. It also activates Rattled.
...
#define B_WEATHER_FORMS             GEN_7 // In Gen5+, Castform and Cherrim revert to their base form upon losing their respective ability. Cherrim needs Flower Gift to swap forms.
#define B_SYMBIOSIS_GEMS            GEN_7 // In Gen7+, Symbiosis passes an item after a gem-boosted attack. Previously, items are passed before the gem-boosted attack hits, making the item effect apply.
#define B_ABSORBING_ABILITY_STRING  GEN_7 // In Gen5+, the abilities that absorb moves of a certain type use a generic string for stat increases and decreases.
#define B_REDIRECT_ABILITY_IMMUNITY GEN_7 // In Gen5+, Pokémon with Lightning Rod/Storm Drain become immune to Electric/Water-type moves and increase their Sp. Attack by 1 stage on top of the redirecting effect.
#define B_REDIRECT_ABILITY_ALLIES   GEN_7 // In Gen4+, Lightning Rod/Storm Drain redirect ally's moves as well.
#define B_LEAF_GUARD_PREVENTS_REST  GEN_7 // In Gen5+, Leaf Guard prevents the use of Rest in harsh sunlight.
#define B_SNOW_WARNING              GEN_7 // In Gen9+, Snow Warning will summon snow instead of hail.
#define B_TRANSISTOR_BOOST          GEN_7 // In Gen9+, Transistor will only boost Electric-type moves by 1.3x as opposed to 1.5x.
#define B_ILLUMINATE_EFFECT         GEN_7 // In Gen9+, Illuminate prevents accuracy reductions and ignores the target's evasion.
#define B_WEAK_ARMOR_SPEED          GEN_7 // In Gen7+, Weak Armor raises Speed by 2 stages instead of 1 when hit by a physical move.
#define B_PROTEAN_LIBERO            GEN_7 // In Gen9+, Protean and Libero change the user's type only once per Battle.
#define B_INTREPID_SWORD            GEN_7 // In Gen9+, Intrepid Sword raises Attack by one stage only once per Battle.
#define B_DAUNTLESS_SHIELD          GEN_7 // In Gen9+, Dauntless Shield raises Defense by one stage only once per Battle.
...
#define B_ABILITY_TRIGGER_CHANCE    GEN_7 // In Gen3, Shed Skin, Cute Charm, Flame Body, Static and Poison Point have a 1/3 chance to trigger. In Gen 4+ it's 30%.
...
#define B_PICKUP_WILD               GEN_7 // In Gen9+, Pickup allows its user to pickup its own used item at the end of the turn in wild battles.
#define B_MAGIC_GUARD               GEN_7 // In Gen4 only, Magic Guard ignores immobilization caused by paralysis
...
#define B_MENTAL_HERB               GEN_7 // In Gen5+, the Mental Herb cures Taunt, Encore, Torment, Heal Block, and Disable in addition to Infatuation from before.
#define B_TRAINERS_KNOCK_OFF_ITEMS  TRUE       // If TRUE, trainers can steal/swap your items (non-berries are restored after battle). In vanilla games trainers cannot steal items.
#define B_RETURN_STOLEN_NPC_ITEMS   GEN_7 // In Gen5+, Thief and Covet no longer steal items from NPCs.
...
#define B_RESTORE_HELD_BATTLE_ITEMS GEN_7 // In Gen9, all non-berry items are restored after battle.
#define B_SOUL_DEW_BOOST            GEN_7 // In Gens3-6, Soul Dew boosts Latis' Sp. Atk and Sp. Def. In Gen7+ it boosts the power of their Psychic and Dragon type moves instead.
#define B_NET_BALL_MODIFIER         GEN_7 // In Gen7+, Net Ball's catch multiplier is x5 instead of x3.
#define B_DIVE_BALL_MODIFIER        GEN_7 // In Gen4+, Dive Ball's effectiveness increases by x3.5 when Surfing or Fishing.
...
#define B_DREAM_BALL_MODIFIER       GEN_7 // In Gen8+, Dream Ball's catch multiplier is x4 when the target is asleep or has the ability Comatose.
...
#define B_IRON_BALL                 GEN_7 // In Gen5+, Flying-type Pokemon holding Iron Ball take x1 damage from Ground-type moves regardless of their other types, except during Inverse Battles or if the Pokemon is grounded by any other effect.
...
#define B_SECRET_POWER_EFFECT       GEN_7 // Secret Power's effects change depending on terrain and generation. See MOVE_EFFECT_SECRET_POWER's case in `SetMoveEffect`.
#define B_SECRET_POWER_ANIMATION    GEN_7 // Secret Power's animations change depending on terrain and generation.
#define B_NATURE_POWER_MOVES        GEN_7 // Nature Power calls different moves depending on terrain and generation. See sNaturePowerMoves.
...
#define B_CATCH_SWAP_INTO_PARTY     GEN_7 // In Gen 7+, the option to swap the caught wild mon to the party will appear, allowing you to send a different mon to the box.
...
#define B_MULTI_BATTLE_WHITEOUT         GEN_7 // In Gen4+, multi battles end when the Player and also their Partner don't have any more Pokémon to fight.
#define B_EVOLUTION_AFTER_WHITEOUT      GEN_7 // In Gen6+, Pokemon that qualify for evolution after battle will evolve even if the player loses.
...
#define B_TRAINER_CLASS_POKE_BALLS      GEN_7 // In Gen7+, trainers will use certain types of Poké Balls depending on their trainer class.
...
#define B_OVERWORLD_SNOW                GEN_7 // In Gen9+, overworld Snow will summon snow instead of hail in battle.
#define B_OVERWORLD_FOG                 GEN_7 // In Gen8+, overworld Fog summons Misty Terrain in battle. In Gen4 only, overworld Fog summons the unique fog weather condition in battle.
#define B_TOXIC_REVERSAL                GEN_7 // In Gen5+, bad poison will change to regular poison at the end of battles.
#define B_TRY_CATCH_TRAINER_BALL        GEN_7 // In Gen4+, trying to catch a Trainer's Pokémon does not consume the Poké Ball.
...
#define B_ENEMY_THROW_BALLS          GEN_7  // In GEN_6+, enemy Trainers throw Poké Balls into battle instead of them just appearing on the ground and opening.
#define B_ENEMY_THROW_BALLS_SOUND    GEN_7  // In GEN_5+, enemy Trainer's Poké Balls make a sound when thrown to send out a Pokémon. This can only be used when B_ENEMY_THROW_BALLS is set to GEN_6 or later.
#define B_PLAYER_THROW_BALLS_SOUND   GEN_7  // In GEN_5+, the player's Poké Balls make a sound when thrown to send out a Pokémon.
...
#define B_ENEMY_MON_SHADOW_STYLE        GEN_7 // In Gen4+, all enemy Pokemon will have a shadow drawn beneath them.

//  Battle UI settings
#define B_MOVE_REARRANGEMENT_IN_BATTLE  GEN_7  //  In Gen 4+ move slots cannot be rearranged in battle
